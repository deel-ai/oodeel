...
class VIM(OODModel):
    """
        ...
        The PCA residual score is based on the projection on residual dimensions for principal component analysis.
            Residual dimensions are the eigenvectors corresponding to the least eignevalues (least variance).
            Intuitively, this score method assumes that feature representations of ID data occupy a low dimensional affine subspace :math:'P+c' of the feature space. 
        ...
        Args:
            princ_dims: number of principal dimensions of in distribution features to consider. 
                ...
                Defaults to None.
            pca_origin: either "center" for using the mean of the data in feature space, or "pseudo" for using W^{-1}b where W^{-1} is the pseudo inverse of the final linear layer applied to bias term (as in the VIM paper).
                Defaults to "center".
            output_layers_id: features to use for Residual and Energy score. 
                Defaults to [-2,-1] (-2 the features for PCA residual, -1 the logits with output_activation="linear" for Energy).
    ...
    def _fit_to_dataset(
        ...
        """
        Computes principal components of feature representations and store the residual eigenvectors.
        Computes a scaling factor constant :math:'\alpha' such that the average scaled residual score (on train) is equal to the average maximum logit score (MLS) score.
        ...
        if self.pca_origin=="center":
            ...
            raise NotImplementedError('only "center" and "pseudo" are available for argument "pca_origin"')
        ...
        if self._princ_dim is None:
            ...
            # we use kneedle to look for an elbow point to set the number of principal dimensions
            # we apply kneedle to the function cumsum(eigvals) which maps a dimension d to the variance of the d dimensional (principal) subspace with lowest variance.
            # since eigvals is non decreasing, cumsum(eigvals) is always convex and increasing.   
            self.kneedle = KneeLocator(range(len(eig_vals)), np.cumsum(eig_vals), S=1.0, curve="convex", direction="increasing")
            ...
            assert self._princ_dim<self.feature_dim, f"if 'princ_dims'(={self._princ_dim}) is an int, it must be less than feature space dimension ={self.feature_dim})"
            self.res_dim=self.feature_dim-self._princ_dim
            self._princ_dim=self._princ_dim
        elif isinstance(self._princ_dim, float):
            assert 0<=self._princ_dim and self._princ_dim<1,  f"if 'princ_dims'(={self._princ_dim}) is a float, it must be in [0,1)"   
    ...
    def plot_spectrum(self)-> None:
        if hasattr(self, "kneedle" ):
            ...
            plt.axvline(x=self.res_dim,color="r", linestyle="--", label=f"Number of principal dimensions = {self._princ_dim} ")
            plt.legend()
            plt.ylabel("Explained variance")
            plt.xlabel("Number of principal dimensions")
            plt.title(f"Explained variance by number of principal dimensions")
